...
type StatusBadgeProps =
  | { status: "Synced" | "OutOfSync" | "Progressing" | "Degraded" | "Healthy" | "Unknown" | "Suspended" }
...
const syncConfig: Record<SyncStatus, { label: string; className: string; dotClass: string }> = {
  ...
  error: { label: "Error", className: "bg-red-500/20 text-red-400 border-red-500/30", dotClass: "bg-red-400" },
}
...
const healthConfig: Record<HealthStatus, { label: string; className: string; dotClass: string }> = {
  ...
  unhealthy: { label: "Unhealthy", className: "bg-red-500/20 text-red-400 border-red-500/30", dotClass: "bg-red-400" },
  unknown: { label: "Unknown", className: "bg-zinc-500/20 text-zinc-400 border-zinc-500/30", dotClass: "bg-zinc-400" },
}
...
export function StatusBadge(props: StatusBadgeProps) {
  ...
  if (!config) {
    config = { label: "Unknown", className: "bg-zinc-500/20 text-zinc-400 border-zinc-500/30", dotClass: "bg-zinc-400" }
  }
