...
type healthWaitingDeployer struct {
	stubDeployer
	health    agentdocker.HealthState
	waitedFor string
}

func (d *healthWaitingDeployer) WaitForApplicationHealth(_ context.Context, appID string) agentdocker.HealthState {
	d.waitedFor = appID
	return d.health
}

func TestExecuteDeployment_ReportsHealthAfterSuccessfulDeploy(t *testing.T) {
	sender := &stubSender{sent: make(chan *messages.ClientMessage, 2)}
	deployer := &healthWaitingDeployer{health: agentdocker.HealthHealthy}

	executeDeployment(context.Background(), sender, deployer, &messages.DeployRequest{
		RequestId:       "req-1",
		ApplicationId:   "app-1",
		ApplicationName: "billing",
		ComposeFile:     "services: {}\n",
	...
	first := <-sender.sent
	if result := first.GetDeployResult(); result == nil || !result.Success {
		t.Fatalf("expected successful DeployResult first, got %v", first.Payload)
	}

	second := <-sender.sent
	report := second.GetApplicationStatusReport()
	if report == nil {
		t.Fatalf("expected ApplicationStatusReport after deploy, got %T", second.Payload)
	}
	...
	if deployer.waitedFor != "app-1" {
		t.Errorf("expected health wait for app-1, got %q", deployer.waitedFor)
	}
}

func TestExecuteDeployment_NoHealthReportOnFailure(t *testing.T) {
	sender := &stubSender{sent: make(chan *messages.ClientMessage, 2)}
	deployer := &healthWaitingDeployer{health: agentdocker.HealthHealthy}
	deployer.err = errors.New("compose up failed")
	...
	first := <-sender.sent
	if result := first.GetDeployResult(); result == nil || result.Success {
		t.Fatalf("expected failed DeployResult, got %v", first.Payload)
	}

	select {
	case msg := <-sender.sent:
		t.Fatalf("expected no health report after failed deploy, got %T", msg.Payload)
	default:
	}
	if deployer.waitedFor != "" {
		t.Errorf("expected no health wait after failed deploy, waited for %q", deployer.waitedFor)
	}
}
