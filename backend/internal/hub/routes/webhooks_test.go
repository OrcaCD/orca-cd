...
// --- isContainerPushEvent ---

func TestIsContainerPushEvent(t *testing.T) {
	tests := []struct {
		name     string
		provider models.RepositoryProvider
		header   string
		...
		{
			name:     "github/CONTAINER/published",
			provider: models.GitHub,
			header:   "package",
			body:     `{"action":"published","package":{"package_type":"CONTAINER"}}`,
			want:     true,
		},
		{
			name:     "github/CONTAINER/updated",
			provider: models.GitHub,
			header:   "package",
			body:     `{"action":"updated","package":{"package_type":"CONTAINER"}}`,
			want:     true,
		},
		{
			name:     "github/CONTAINER/lowercase_type",
			provider: models.GitHub,
			header:   "package",
			body:     `{"action":"published","package":{"package_type":"container"}}`,
			want:     true,
		...
		},
		{
			...
			provider: models.GitHub,
			header:   "package",
			body:     `{"action":"published","package":{"package_type":"NPM"}}`,
			want:     false,
		},
		{
			name:     "github/wrong_event_header",
			provider: models.GitHub,
			header:   "push",
			body:     `{"action":"published","package":{"package_type":"CONTAINER"}}`,
			want:     false,
		...
		{
			name:     "gitlab/not_supported",
			provider: models.GitLab,
			header:   "",
			body:     `{}`,
			want:     false,
		},
		{
			name:     "gitea/not_supported",
			provider: models.Gitea,
			header:   "",
			body:     `{}`,
			want:     false,
		},
		{
			name:     "generic/not_supported",
			provider: models.Generic,
			header:   "",
			body:     `{}`,
			want:     false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.Request = httptest.NewRequest(http.MethodPost, "/", nil)
			if tt.header != "" {
				c.Request.Header.Set("X-GitHub-Event", tt.header)
			}
			got := isContainerPushEvent(c, tt.provider, []byte(tt.body))
			if got != tt.want {
				t.Errorf("isContainerPushEvent(%s) = %v, want %v", tt.name, got, tt.want)
			}
		})
	}
}

// --- WebhookHandler container registry integration tests ---

func seedAppForWebhookRepo(t *testing.T, repoID, agentID string) models.Application {
	t.Helper()
...
func TestWebhookHandler_GitHub_ContainerPushEvent_SendsPullImagesRequest(t *testing.T) {
	setupTestDBWithWebhookRepos(t)
	hub := setupHubForTest(t)

	const secret = "mysecret"
	const agentID = "agent-container-test"
	const body = `{"action":"published","package":{"package_type":"CONTAINER"}}`

	repo := seedWebhookRepo(t, models.GitHub, secret)
	app := seedAppForWebhookRepo(t, repo.Id, agentID)
	...
	// Sync status must NOT be updated (unlike git push events)
	var got models.Repository
	if err := db.DB.First(&got, "id = ?", repo.Id).Error; err != nil {
		t.Fatalf("failed to reload repo: %v", err)
	}
	if got.SyncStatus != models.SyncStatusUnknown {
		t.Errorf("expected sync_status unchanged, got %s", got.SyncStatus)
	}
}

func TestWebhookHandler_GitHub_ContainerPushEvent_Updated_SendsPullImagesRequest(t *testing.T) {
	setupTestDBWithWebhookRepos(t)
	hub := setupHubForTest(t)

	const secret = "mysecret"
	const agentID = "agent-container-updated"
	const body = `{"action":"updated","package":{"package_type":"CONTAINER"}}`

	repo := seedWebhookRepo(t, models.GitHub, secret)
	_ = seedAppForWebhookRepo(t, repo.Id, agentID)
...
func TestWebhookHandler_GitHub_ContainerPushEvent_NonContainer_Ignored(t *testing.T) {
	setupTestDBWithWebhookRepos(t)
	setupHubForTest(t)
...
func TestWebhookHandler_GitHub_ContainerPushEvent_InvalidSignature(t *testing.T) {
	setupTestDBWithWebhookRepos(t)
...
func TestWebhookHandler_GitHub_ContainerPushEvent_NoApps_Returns204(t *testing.T) {
	setupTestDBWithWebhookRepos(t)
	setupHubForTest(t)
